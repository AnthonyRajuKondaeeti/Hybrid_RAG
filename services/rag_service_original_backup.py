    ...
    def answer_question(self, file_id: str, question: str, 
        ...
        try:
            ...
            with performance_timer("Answer generation", self.timing_stats):
                ...
                enhanced_prompt = self.create_enhanced_anti_hallucination_prompt(
                    question, context_chunks, content_type
                )
                
                # Generate with enhanced prompt
                generation_result = self._generate_enhanced_answer_with_prompt(
                    enhanced_prompt, file_chunks, conversation_history, conversation_context
                )
                
                # Validate answer quality
                if generation_result['success'] and 'answer' in generation_result:
                    # Convert chunks to strings for quality validation
            ...
            # Calculate confidence (enhanced with quality metrics)
...
Please provide a brief summary of what information is available."""
    ...
    def _generate_enhanced_answer_with_prompt(self, enhanced_prompt: str, search_results: List[SearchResult],
                                           conversation_history: List[Dict] = None, 
        ...
        try:
            ...
            # Minimal post-processing to preserve anti-hallucination structure
    ...
    def _generate_answer_with_llm(self, prompt: str) -> str:
        """Generate answer using LLM with basic error handling"""
        try:
            response = self.llm.invoke(prompt)
    ...
    def create_anti_hallucination_prompt(self, question, context_chunks, content_type="general"):
        """Create prompts that minimize hallucinations"""
        ...
        if "healthcare" in content_type.lower() or any(term in question.lower() for term in ['hba1c', 'diabetes', 'blood pressure']):
            prompt_template = AntiHallucinationPrompts.HEALTHCARE_PROMPT
        elif "financial" in content_type.lower() or any(term in question.lower() for term in ['revenue', 'profit', 'quarter']):
            prompt_template = AntiHallucinationPrompts.FINANCIAL_PROMPT
        else:
            prompt_template = AntiHallucinationPrompts.STRICT_GROUNDING_PROMPT
        
        return prompt_template.format(context=numbered_context, question=question)

    def validate_answer_quality(self, answer, source_chunks):
        """Enhanced quality validation to catch hallucinations and improve accuracy"""
        
        if not answer or len(answer.strip()) < 10:
            return {"quality": "poor", "score": 0.0, "issues": ["Answer too short"]}
        ...
        source_text = ' '.join(source_chunks).lower() if isinstance(source_chunks, list) else str(source_chunks).lower()
        
        for number in numbers_in_answer:
            if number not in source_text:
                issues.append(f"Number '{number}' not found in sources")
        
        # Check for specific medical/financial terms that need precise grounding
        medical_terms = ['hba1c', 'blood pressure', 'diabetes', 'medication', 'dosage']
        financial_terms = ['revenue', 'profit', 'quarter', 'earnings', 'percentage']
        
        for term in medical_terms + financial_terms:
            if term in answer_lower and term not in source_text:
                issues.append(f"Specialized term '{term}' mentioned but not found in sources")
        
        # Check for source citations
        has_citations = bool(re.search(r'\[Source \d+\]', answer))
        if not has_citations and len(source_chunks) > 0:
        ...
        return {
            ...
            "score": quality_score,
            "issues": issues,
            "validation_passed": len(issues) <= 2,
            "hallucination_risk": 1.0 - quality_score,
            "has_citations": has_citations
        }

    def _detect_content_type(self, question: str, first_chunk: str) -> str:
        """Detect content type for specialized handling"""
        text = (question + " " + first_chunk).lower()
        
        if any(term in text for term in ['hba1c', 'diabetes', 'blood pressure', 'medication', 'medical', 'patient', 'clinical', 'diagnosis', 'treatment']):
            return "healthcare"
        elif any(term in text for term in ['revenue', 'profit', 'quarter', 'financial', 'business', 'earnings', 'budget', 'cost']):
            return "financial"
        elif any(term in text for term in ['api', 'system', 'technical', 'architecture', 'requests', 'server', 'database']):
            return "technical"
        elif any(term in text for term in ['.csv', 'column', 'row', 'data', 'table', 'spreadsheet']):
            return "csv_data"
        else:
            return "general"

    def create_enhanced_anti_hallucination_prompt(self, question: str, context_chunks: List[str], content_type: str = "general") -> str:
        ...
        # Choose prompt based on content type
        if content_type == "healthcare":
            prompt_template = AntiHallucinationPrompts.HEALTHCARE_PROMPT
        elif content_type == "financial":
            prompt_template = AntiHallucinationPrompts.FINANCIAL_PROMPT
        elif content_type == "csv_data":
            prompt_template = AntiHallucinationPrompts.CSV_DATA_PROMPT
        else:
            prompt_template = AntiHallucinationPrompts.STRICT_GROUNDING_PROMPT
        
        return prompt_template.format(context=numbered_context, question=question)

    def answer_question_improved(self, file_id: str, question: str, 
                               conversation_history: List[Dict] = None, 
                               session_id: str = None) -> Dict[str, Any]:
        """Enhanced answer generation with hallucination reduction and quality validation"""
        ...
        try:
            ...
            for result in search_results:
                content = result.chunk.content.strip() if hasattr(result.chunk, 'content') else str(result).strip()
                if content and len(content) > 20:
                    context_chunks.append(content)
            ...
            # 3. Detect content type for specialized handling
            content_type = self._detect_content_type(question, context_chunks[0])
            
            # 4. Create enhanced anti-hallucination prompt
            improved_prompt = self.create_enhanced_anti_hallucination_prompt(
                question, context_chunks, content_type
            )
            
            # 5. Generate answer with existing LLM
            raw_answer = self._generate_answer_with_llm(improved_prompt)
            
            # 6. Validate answer quality
            quality_check = self.validate_answer_quality(raw_answer, context_chunks)
            
            # 7. Return enhanced response with quality metrics
            return {
                "success": True,
                "answer": raw_answer,
                "confidence_score": quality_check["score"],
                "quality_metrics": {
                    "quality_grade": quality_check["quality"],
                    "validation_passed": quality_check["validation_passed"],
                    "issues_detected": quality_check["issues"],
                    "hallucination_risk": quality_check["hallucination_risk"],
                    "has_citations": quality_check["has_citations"],
                    "content_type": content_type
                },
                "sources": [
                    {
                        "content_preview": chunk[:200] + "..." if len(chunk) > 200 else chunk,
                        "relevance_score": search_results[i].hybrid_score if i < len(search_results) else 0.0
                    }
            ...
            logger.error(f"Error in improved answer generation: {e}")
            return {
                "success": False,
                "error": str(e),
                "answer": "I encountered an error processing your question. Please try again.",
                "confidence_score": 0.0,
                "quality_metrics": {"quality_grade": "error"}
            }

    def _create_no_results_response(self, question: str) -> Dict[str, Any]:
        """Better no-results response"""
        return {
            "success": True,
            "answer": "I couldn't find relevant information in the document collection to answer your question. Please ensure the relevant documents are uploaded or try rephrasing your question.",
            "confidence_score": 0.0,
            "quality_metrics": {"quality_grade": "no_results"},
            "sources": []
        }
...
Provide a concise, direct answer (maximum 3-4 sentences):"""
    ...
    def _prepare_final_response(self, generation_result: Dict[str, Any], 
                               ...
                               start_time: float) -> Dict[str, Any]:
        """Prepare final response with all metadata including quality metrics"""
        processing_time = time.time() - start_time
        ...
        if 'quality_metrics' in generation_result:
            response['quality_metrics'] = generation_result['quality_metrics']
            response['anti_hallucination_enabled'] = True
